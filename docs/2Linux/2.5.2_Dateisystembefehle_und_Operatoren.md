# 2.5.1 Dateisystembefehle und Operatoren
...
!!! task "Spiel: Dateiverwaltung und Links"
    ...
    1. Erstellen Sie eine Datei namens `ersteDatei` und eine weitere Datei namens `zweiteDatei`. Hier wird verraten, dass dies wie folgt geht: `touch ersteDatei zweiteDatei`
    2. Kopieren Sie `ersteDatei` nach `zweiteDatei`.
    3. Benennen Sie `ersteDatei` in `neueDatei` um und nutzen Sie das Argument `-i`.
    4. Kopieren Sie `neueDatei` in eine versteckte Datei namens `.versteckteDatei`.
    5. Kopieren Sie `neueDatei` nach `weitereDatei` und nutzen Sie `-v`.
    6. Kopieren Sie `neueDatei` nach `zusätzlicheDatei` und nutzen Sie `-i`.
    7. Kopieren Sie `weitereDatei` nach `.versteckteDatei` und überschreiben Sie sie und nutzen Sie `-iv`.
    8. Kopieren Sie `neueDatei` nach `weitereDatei` und sichern Sie die alte Version mit `-b`.
    9. Schreiben Sie den Text "test" in `weitereDatei`.
    10. Kopieren Sie `neueDatei` nach `weitereDatei` und sichern Sie die alte Version mit `-b`.
    11. Erstellen Sie einen symbolischen Link namens `neuerLink`, der auf `neueDatei` verweist.
    12. Kopieren Sie `neuerLink` nach `neuerLink2`.
    13. Löschen Sie alle Dateien, die mit `Datei` enden. Hier wird verrate, dass dies mit `rm *Datei` geht.
    14. Beantworten Sie folgende Fragen:
