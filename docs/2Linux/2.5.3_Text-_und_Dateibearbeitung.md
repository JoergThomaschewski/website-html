# 2.5.2 Text- und Dateibearbeitung
...
Hier sind wir im Bereich der Nerd-Befehle: mächtig und etwas verrückt. Wenn man wirklch mit einem Linux-System arbeite, dann sind diese Befehle sehr hilfreich.
...
!!! note "Text für die Aufgabe"
    ...
    Source-Entwicklung verbracht. Mitte der Achtziger war ich einer der ersten Beitragenden zu en GNU. Ich hatte bereits umfangreiche Open 
    Source-Software im Internet veröffentlicht, die ich selbst entwickelt oder mitentwickelt hatte (en nethack, en  Emacs VC (Version Control) 
    und GUD modes, xlife und andere) und die heute noch viel verwendet wird. Ich dachte, ich wüsste, wie es gemacht wird.

    Dann stellte Linux alles in Frage, was ich zu wissen glaubte. Ich hatte das Unix-Evangelium der kleinen Tools, des de rapid prototyping 
    ...
    wichtigste Software (Betriebssysteme und wirklich umfangreiche Tools wie en Emacs) so gebaut werden müssten wie Kathedralen, sorgsam
    ...
    de Linus Torvalds Entwicklungsstil auf der anderen Seite - mit seinen frühen und häufigen Freigaben, seinem Delegieren von allem, was nur 
...
5. Zeigen Sie die ersten 5 Zeilen der Datei `spiel_kopie.txt` mit `head` an.
